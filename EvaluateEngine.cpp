      ...
      if (kingMoveBlack[i][j] == BLACK) {
        ...
        while (!que.empty()) {
          ...
          for (int k = 0; k < 8; k++) {
            m = pos1.x + offset[k][0];
            n = pos1.y + offset[k][1];
            if (m >= 0 && n >= 0 && m < 10 && n < 10 && visit[m][n] == 0 &&
                kingMoveBlack[m][n] >= 0) {
              visit[m][n] = 1;  //标记为访问过的
              pos.x = m;
              pos.y = n;
      ...
      if (kingMoveWhite[i][j] == WHITE) {
        ...
        while (!que.empty()) {
          ...
          for (int k = 0; k < 8; k++) {
            m = pos1.x + offset[k][0];
            n = pos1.y + offset[k][1];
            if (m >= 0 && m < 10 && n >= 0 && n < 10 && visit[m][n] == 0 &&
                kingMoveWhite[m][n] >= 0) {
              visit[m][n] = 1;  //标记为访问过的
              pos.x = m;
              pos.y = n;
        ...
        while (!que.empty()) {
          ...
          for (int k = 0; k < 8; k++) {
            ...
            while (m >= 0 && m < 10 && n >= 0 && n < 10 &&
                   queenMoveBlack[m][n] >= 0) {
              if (queenMoveBlack[m][n] == 0 ||
  ...
  for (int i = 0; i < 10; i++) {
    for (int j = 0; j < 10; j++) {
      if (Board[i][j] == EMPTY) {
        ...
        // 直接比较浮点数不安全，使用做差的方式比较合适，其他地方都是这样
        if (fabs(q1 - D_INF) < eps && fabs(q2 - D_INF) < eps) {
          t1 += 0;
        } else if (q1 < D_INF && fabs(q1 - q2) < eps) {
          t1 += K;
        } else if (q1 < q2) {
          t1 += 1;
        } else if (q1 > q2) {
    ...
    for (int j = 0; j < 10; j++) {
      ...
      //白棋的灵活度
        ...
        for (int k = 0; k < 8; k++) {
          int a = i + offset[k][0], b = offset[k][1];
          while (a >= 0 && a < 10 && b >= 0 && b < 10 && Board[a][b] == EMPTY) {
            if (kingMoveWhite[a][b] == 0)
              continue;  //处理浮点数异常，暂时没有解决

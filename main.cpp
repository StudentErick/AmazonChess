...
unsigned long long int randomInt() {
  uniform_int_distribution<unsigned long long int> dist(0, UINT32_MAX);
  return dist(mt);
}
/*
...
void initTable() {
  for (int i = 0; i < 8; i++)
    for (int j = 0; j < 8; j++)
      for (int k = 0; k < 12; k++) ZobristTable[i][j][k] = randomInt();
}
